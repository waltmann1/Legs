...
class BasicSV40Body(Body):

    def __init__(self, index=0):
        super(BasicSV40Body, self).__init__(index=0)
        r = 4
        ir = 2
        ...
        for i in range(0,5):
            ...
            self.binding_sites.append([r * np.sin(2 * np.pi / 5 * i + np.pi / 10), r * np.cos(2 * np.pi / 5 * i + np.pi / 10), -.5])
